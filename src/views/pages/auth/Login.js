...
const LoginV1 = () => {
  ...
  const [showError, setShowError] = useState('hide-error-msg')
  const [showErrorContent, setShowErrorContent] = useState('')
  ...
  const handleSubmit = event => {
    ...
    if (loginemail !== '' && loginpassword !== '') {
      ...
      const config = {
        method: 'post',
        url: 'https://digital-oasis-dev.herokuapp.com/v3/user/login',
        headers: { 
      ...
      .then(function (response) {
        ...
        if (response.data.status === 200) {
          ...
          if (user && user.auth_token) {
            ...
            toast.success(
              <ToastContent name={user.data.first_name || 'John Doe'} />,
              { icon: false, transition: Slide, hideProgressBar: true, autoClose: 2000 }
            )
          }
        } else if (response.data.status === 400) {
          setShowError('show-error-msg')
          setShowErrorContent(response.data.message)
      ...
      setShowErrorContent('Invalid email or password')
      setShowError('show-error-msg')
    }
        ...
        <Card className='mb-0'>
          <CardBody>
            ...
            <CardTitle tag='h4' className='mb-1'>
              Welcome to Digital Oasis
            </CardTitle>
            <CardText className='mb-2'>Please sign-in to your account</CardText>
            <p className={showError}>{showErrorContent}</p>
